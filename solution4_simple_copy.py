...
group1 = list(range(0,24))
group2 = list(range(24,48))
group3 = list(range(48,73))
...
papers = torch.unique(torch.concat([cited_papers, subject_papers])).to(device)
subgraph = dgl.node_subgraph(G, {'subject':subjects,'author':authors, 'paper':papers})
# print(subgraph.nodes('subject'))

# print("size", papers.shape)
...
model = HGT(subgraph, n_inp=400, n_hid=200, n_out=labels.max().item()+1, n_layers=2, n_heads=4, use_norm = True).to(device)
...
#model = HGT(subgraph, n_inp=400, n_hid=200, n_out=labels.max().item()+1, n_layers=2, n_heads=4, use_norm = True)
optimizer = torch.optim.AdamW(model.parameters())
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, total_steps=100, max_lr = 1e-3 /2, pct_start=0.05)
